<template>
	...
	<div class="btn-group">
		...
		<a id="status">
			<span title="Status" :class="{'btn btn-success btn-sm':data.status !== 0,'btn btn-danger btn-sm':data.status === 0}" id="edit_btn">{{data.order.status ? lang('active') : lang('pending')}}</span>
		</a>
