...
import { generateCurrencyCountryMap } from './currencyCountryMap';
...
export const getSubStringValue = (name,count) => {
	if(name){
		...
		if(txt.value.length>count){
			return txt.value.substring(0,count) + '...';
		} else {
			return txt.value;
		}
	}
...
const getCurrencySymbol = (currencyCode) => {
	const currencyCountryMap = generateCurrencyCountryMap();
	const currencyObject = findObjectByKey(currencyCountryMap, 'currency', currencyCode);
	return currencyObject ? currencyObject.symbol : '$'; // Default to USD symbol if not found
};
...
/**
 * `customImageUploadHandler` function handles image uploads in TinyMCE editor
 *  Parameters:
 *  - `blobInfo`: Provides information about the image blob to be uploaded.
 *  - `progress`: A callback function to update the upload progress bar.
 *  - `url`: The API endpoint URL for uploading the image.
 */
export const customImageUploadHandler = (blobInfo, progress, url) =>
	new Promise((resolve, reject) => {
		const xhr = new XMLHttpRequest(); // Create a new XMLHttpRequest object for making HTTP requests.
		xhr.withCredentials = false; // Do not include credentials such as cookies in the request.
		xhr.open('POST', url); // Open a POST request to the provided URL.

		// Upload progress event listener
		xhr.upload.onprogress = (e) => {
			// Update the progress with percentage completion of the upload.
			progress((e.loaded / e.total) * 100);
		};

		// When the upload is complete
		xhr.onload = () => {
			// If the server responds with a 403 (Forbidden) status
			if (xhr.status === 403) {
				reject({ message: 'HTTP Error: ' + xhr.status, remove: true });
				return;
			}

			// If the server responds with a 422 (Unprocessable Entity) or 412 (Precondition Failed) status
			if (xhr.status === 422 || xhr.status === 412) {
				const response = JSON.parse(xhr.responseText); // Parse the error response as JSON.
				reject(response.message.file ? response.message.file : response.message); // Reject the promise with the error message.
				return;
			}
			...
			// Validate if the response contains a `location` key with a string value
			if (!json || typeof json.location !== 'string') {
				reject('Invalid JSON: ' + xhr.responseText); // Reject if the response is not as expected.
				return;
			}

			// Resolve the promise with the image URL (location) returned from the server
		...
		formData.append('file', blobInfo.blob(), blobInfo.filename()); // Append the image file with its name.
...
/**
 * `getSystemLanguage` function maps the application's language code to TinyMCE supported language codes.
 *    Parameter:
 *  - `appLang`: The application's language code (e.g., 'en', 'fr', 'zh-hans') bsn and mt lang were need to be added.
 */

export const getSystemLanguage = (appLang) => {
	// Define a mapping of application language codes to TinyMCE language codes.
	const languageMap = {
		'ar': 'ar',         // Arabic
		'de': 'de',         // German
		'es': 'es',         // Spanish
		...
		'pt': 'pt_PT',      // Portuguese
		'ru': 'ru',         // Russian
		'zh-hans': 'zh_TW', // Simplified Chinese
		'zh-hant': 'zh_CN', // Traditional Chinese
		'no': 'nb_NO',      // Norwegian
		'vi': 'vi'          // Vietnamese
	};

	// Return the mapped TinyMCE language code or default to 'en' (English) if not found.
	return languageMap[appLang] || 'en';
};

/**
 * Replaces all emoji shortcodes in the given content with their corresponding emojis.
 * @param {string} content - The content containing emoji shortcodes.
 * @return {string} - The content with emoji shortcodes replaced by emojis.
 */
export const renderLabelWithEmoji = (content) => {
	// Create a map of shortcodes to native emojis
	const emojiMap = emojiData.reduce((map, emoji) => {
		emoji.short_names.forEach(short_name => {
			map[`:${short_name}:`] = String.fromCodePoint(...emoji.unified.split('-').map(u => '0x' + u));
		});
		return map;
	}, {});

	// Replace shortcodes with emojis
	const emojifiedContent = content.replace(/:([a-zA-Z0-9_+-]+):/g, (match) => {
...
export const getQueryParams = (params) => {
	...
	let queryString = Object.entries({...params, ...urlParams}).map(([key, value]) => {
