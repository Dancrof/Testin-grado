<template>
	...
	<parsed-data :dataValue="data" :from="from" @click.native="handleAnchorClick"></parsed-data>
...
export default {
    ...
    from: { type: String, default: 'activity-parser' },
	},
	...
	beforeMount(){
		...
		app.component('parsed-data',{
          ...
          // Check if emoji short code is present before replacing only loaded emoji in inbox ticket activity
          if (/:([a-zA-Z0-9_]+):/.test(root.innerHTML) && (this.from === 'ticket-activity')) {
            // Replace emoji short codes with actual emojis in the entire content
            root.innerHTML = renderLabelWithEmoji(root.innerHTML);
          }
