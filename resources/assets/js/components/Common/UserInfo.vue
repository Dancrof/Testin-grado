...
<template>
 <span>
  <a class="user_name pop" :href="showUser(from)" data-container="body" data-bs-toggle="popover" data-placement="  right" :data-content="hoverContent" :data-title="'<label '+hari+'>User Info</label>'">
  ...
  export default{
   ...
   computed:{
     ...
     hoverContent(){
      ...
      return  boolean(this.from) ? "<div class='box-widget widget-user-2 cus-container'><div class='widget-user-header text-bg-blue custom-head'><div class='widget-user-image'><img class='img-circle user-img' alt='User Avatar' src='"+this.from.profile_pic+"'></div><h3 class='widget-user-username custom-name'><a href='"+window.axios.defaults.baseURL+'/user/'+this.from.id+" '>"+((this.from.first_name !== '') ?  "<span>" + this.from.first_name+" "+this.from.last_name+"</span>" : "<span>"+this.from.user_name+"</span>" )+"</a></h3></div><div class='box-footer no-padding'><ul class='nav nav-stacked'><li id='b_bottom'><a id='cus-link' title='"+this.from.email+"'><i class='fa fa-envelope'></i>&nbsp; <span>"+(this.from.email  ?  "<span>" + this.subString(this.from.email)+"</span>" : "<span>"+"---"+"</span>" )+"</span></a></li></ul></div></div>" : ''
     }
   },
