<template>
  ...
  <div class="col-sm-12 tinyMce-editor" :key="id">
...
export default {
  ...
  props: {
    ...
    onChange: {
      type: Function, required: true, default: (content, name) => {
      }
    },
  ...
  data() {
    return {
      ...
      showMsg: false,
      options: {},
      url: `${this.basePath()}/api/tiny-image-uploader?_token=${document.querySelector('meta[name="csrf-token"]').getAttribute('content')}&auth=false`,
      fetchedContents: [],
      currentIndex: 0,
      languages: [],
      showLoader: false,
      ...
      isTranslated: false,
      editorInstance: null // To store the editor instance for outside usage
    };
  },
  ...
  beforeMount() {
    ...
    this.options = {
      ...
      menu: {
        ...
        insert: {
          title: 'Insert',
          items: 'image link media addcomment pageembed template inserttemplate codesample inserttable accordion | charmap emoticons hr | pagebreak nonbreaking anchor footnotes | mergetags | insertdatetime | customtoc'
        }
      },
      ...
      quickbars_selection_toolbar: 'bold italic | aiIcon | quicklink h2 h3 blockquote quickimage quicktable',
      ...
      auto_focus: self.id && /focus_editor\d*/.test(self.id) ? self.id : '', // Focus the editor if the ID matches the pattern
        ...
        source: function (query, process, delimiter) {
          ...
          if (delimiter === '@') {
            axios.get('/api/dependency/agents?limit=all').then(res => {
              res.data.data.agents.forEach(obj => {
          ...
          if (delimiter === '#') {
            axios.get('/api/dependency/departments?limit=all').then(res => {
              res.data.data.departments.forEach(obj => {
          ...
          if (delimiter === '$') {
            axios.get('/api/dependency/teams?meta=true&limit=all').then(res => {
              res.data.data.teams.forEach(obj => {
      ...
      setup: function (editor) {
        ...
        editor.on('focus', function () {
          ...
          // Remove the suggestion if the elea-suggest element is present
        ...
        // Handle backspace key (keyCode 8) for bold text with trailing spaces
        editor.on('keydown', function (e) {
          if (e.keyCode === 8) { // Backspace key
            const selection = editor.selection;
            const range = selection.getRng();

            // If the selection is collapsed (caret only)
            if (range.collapsed) {
              const currentNode = range.startContainer;
              const previousNode = currentNode.previousSibling;

              // If the previous node is bold (strong or b) and contains only spaces or is empty
              if (previousNode && previousNode.nodeType === Node.ELEMENT_NODE && previousNode.tagName === 'STRONG' && /^\s*$/.test(previousNode.textContent)) {
                e.preventDefault(); // Prevent default backspace action
                previousNode.remove(); // Remove the empty bold node
                return;
              }

              // Check if the current node contains bold text and ends with a space
              const currentText = currentNode.textContent;
              if (currentText && /^\s+$/.test(currentText) && currentNode.parentNode.tagName === 'STRONG') {
                e.preventDefault();
                currentNode.textContent = currentText.slice(0, -1); // Remove one space
                return;
              }
            }
          }
      ...
      toolbar: 'undo redo aiIcon  bold italic underline strikethrough | fontfamily fontsize blocks | align | outdent indent |  numlist bullist | forecolor backcolor removeformat | pagebreak | charmap emoticons | fullscreen  preview save print | insertfile image media template link anchor codesample | ltr rtl'
    };
  },
  created() {
    window.emitter.on('appendSecretMessage', (data) => {
      const secretMessage = `<strong class="mceNonEditable">${this.lang('secret_content')}:</strong><a href="${data.data.url}" target="_blank" class="mceNonEditable"> ${this.lang('click_here_to_view_message')}</a><br><strong class="mceNonEditable">${this.lang('expiration')}</strong> ${data.data.expiration}`;

      this.editorValue = secretMessage + this.editorValue;
    });

    window.emitter.on('show-session-link', (session_link) => {

      this.insertTextAtCursor(session_link);
    })
  },
  ...
  methods: {
    ...
    handleEditorChange() {
      this.onChange(this.editorValue, this.name);
    },
    ...
    generateResponse({solution, tone, translate}) {
      ...
      if (editor) {
        ...
        const handleError = (error) => {
          this.fetchedContents = [error.message];
          this.insertContextToolbar();
          removeLoader();
          this.showLoader = false;
    ...
    insertContent(index) {
      ...
      if (editor) {
        ...
        if (selectedContentElement) {
          ...
          editor.notificationManager.open({
            text: this.lang('content-inserted'),
            type: 'success',
            timeout: 3000
          });
        }
      ...
      if (content && !content.includes('elea-suggest') && this.hasIsSettingActiveGetter && this.isSettingActive('grammar_spell_check')) {
        eleaGrammarCheck(content).then(suggestions => {
          ...
          if (error.response && error.response.data && error.response.data.message) {
            errorMessage = this.lang(error.response.data.message);
          }
    ...
    insertTextAtCursor(text) {
      ...
      setTimeout(() => {
        const selection = this.editorInstance.selection.getBookmark(2);

        // Focus the editor to ensure it can accept input
        this.editorInstance.focus();

        // Restore the selection to the saved position
        this.editorInstance.selection.moveToBookmark(selection);

        // Insert the text at the restored position
        this.editorInstance.selection.setContent(text);
      }, 0)
    }
  },
  ...
  computed: {
    ...
    // Check if all settings are inactive
    ...mapGetters(['isSettingActive', 'areAllSettingsInactive']),
