		...
		<div class="row">
			...
			<ValidationProvider :name="name" :rules="rules" class="row" tag="div" v-model="selectedValue">
				...
				<template v-slot="{ field, errorMessage, meta, classes }">
					...
					<div class="col-md-4 c-form-checkbox" v-for="option in options" :key="option.id">

						<label class="control-label">

							<input type="checkbox" class="form-check-input m-1" :id="option.form_identifier" v-model="selectedValue" :value="option" :name="getUniqueName(option.form_identifier)" @change="onInput(selectedValue)" :class="errorMessage ? 'field-danger' : ''">
...
<style scoped>
	...
	.c-form-checkbox > label { font-weight: 400; cursor: pointer; }
  	...
  	.form-check-input{ margin-left: 1px !important; }
