									...
									<tr v-for="user in filteredUsers" role="row" class="odd">
										...
										<td>
											...
											<router-link :to="{ name:'EditUser', params:{id:user.id} }">
												...
												<button :class="{float1: lang_locale == 'ar'}" class="btn btn-primary btn-sm" title="Edit">
													<i class="fa fa-edit"> </i>
												</button>
											</router-link>
											<a v-if="auth.user_data.id !== user.id" :class="{left: lang_locale == 'ar'}">
												<button class="btn btn-secondary btn-sm" @click="getDeleteId(user.id)" title="Deactivate"
