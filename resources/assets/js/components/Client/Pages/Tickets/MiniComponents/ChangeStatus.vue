<template>
	...
	<div class="btn-group">
		...
		<button type="button" class="btn btn-sm btn-light dropdown-toggle" data-bs-toggle="dropdown" @click="statusList" id="client_status_list">
		...
		<ul class="dropdown-menu dropdown-menu-end status-list" >
			...
			<li v-if="!loading" v-for="status in TicketStatusList" :key="status.id">
				...
				<a class="dropdown-item" :class="{align1: lang_locale == 'ar'}"  v-if="status.allow_client" v-tooltip="status.name"
					href="javascript:void(0)" @click="updateStatus(status)" :id="'client_status_'+status.id">

					<i :class="status.icon" :style="{color:status.icon_color}"></i> {{status.name && status.name.length > 15  ? status.name.substring(0,15) + '...' : status.name }}
				</a>
			</li>
		</ul>
	</div>
