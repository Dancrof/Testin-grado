<template>
    <div :class="[classname, 'form-field-template', 'mb-3', {'has-error': name && errors && (name in errors) }, { 'row': isInlineForm } ]" v-bind:id="label">

        <div :class="{ 'col-md-2 flex break-all': isInlineForm }">
          <label class="form-label" v-bind:for="label" v-tooltip="showTooltipOnLabel ? label : ''" :style="labelStyle">{{getSubString(label,labelLength)}}</label>
          ...
          <a v-if="showNewButton && !isInlineForm" class="btn btn-light btn-sm mb2 float-end" href="javascript:;" @click="clickEvent(name)">
        ...
        <div :class="[ isInlineForm ? 'col-md-10 flex' : actionBtn && actionBtn.isFor === 'requester' ? 'flex-100' : '' ]">
            ...
            <button v-if="actionBtn" type="button" :class="isRequester ? 'req-btn' : ''" class="btn btn-light form-field-action-button" @click="() => actionBtn.action()"><span>{{trans(actionBtn.text)}}</span></button>
	        
	        <a v-if="showNewButton && isInlineForm" class="req-btn btn btn-light form-field-action-button new_entry_btn" href="javascript:;" @click="clickEvent(name)">
