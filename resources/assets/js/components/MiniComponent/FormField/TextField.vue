        ...
        <template v-if="from">
            ...
            <div v-if="type === 'textarea'">
              ...
              <textarea :id="id" :name="name" :class="['form-control', inputClass]" :maxlength="length" :type="type" v-model="changedValue"
                        v-on:input="onChange(changedValue, name)" @keyup="keyupListener($event,name)" autocomplete="on"
            ...
            <ValidationProvider :name="name" :rules="rules" v-model="changedValue">
                ...
                <template v-slot="{ field, errorMessage, meta, classes }">
                    ...
                    <div class="input-group" v-else-if="type === 'password' ">
                        <span v-if="page === 'oauth-login' && type === 'password'" class="input-group-text oauth-login-icon"><i class="fa fa-lock"></i></span>
                        <input
                                :id="id"
                                :name="name"
                                ...
                                :ref="autofocus ? 'textField' : ''"
                        />
                        <span  v-if="type === 'password'" class="input-group-text"> <i :class="['fa', showPassword ? 'fa-eye' : 'fa-eye-slash']" @click="togglePassword"></i></span>
                    </div>
                  <div class="input-group mb-3" v-else-if="type === 'text' && page === 'oauth-login'">
                    ...
                    <input @mouseenter="handelTooltip(true)" @mouseleave="handelTooltip(false)"
                           v-else
                            :id="id"
                            :name="name"
                           :class="['form-control', inputClass, classes, errorMessage ? 'field-danger' : '', isUserNameOrEmail ? 'text-lowercase' : '']"
...
export default {
  ...
  computed: {
    ...
    watch: {
        value(newVal) {
          this.changedValue = (this.id === 'seats' && newVal <= 0 && newVal !== '') ? 0 : this.decodeHtml(newVal);
        },
