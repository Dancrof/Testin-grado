...
{
    /**
     * MySql version less than 8.x are not compatible so if version is
     * between 8.0 and 9(including minor and major tags for 8) then we return true
     */
    if($version >= 80000 && $version < 90000) return true;
...
function checkDBPrerequisites(array &$results, bool &$mysqli_ok, object $connection):void
{
    if (mysqli_select_db($connection, DB_NAME)) {
        ...
        if (compareMySqlAndMariDB($dbVersion)) {
            ...
            $results[] = new TestResult('Your MySQL version is ' . $mysqli_version . '. We recommend upgrading to at least MySQL 8.0 or MariaDB 10.3!', STATUS_ERROR);
            $mysqli_ok = false;
        }
    } else {
